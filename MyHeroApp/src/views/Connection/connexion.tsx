	...
	const uploadImage = (image_uri: string) => {
		...
		fetch(base_url, {
			...
			.then((res: any) => {
				...
				setImg({
					...img,
					uri: `http://176.31.230.112:3000/api/avatar/${state.mail
						.toLowerCase()
						.replace(/^"(.+(?="$))"$/, "$1")}?time=${new Date()}`,
				});
				dispatch(
					setImage(
						`http://176.31.230.112:3000/api/avatar/${state.mail
							.toLowerCase()
							.replace(/^"(.+(?="$))"$/, "$1")}?time=${new Date()}`
					)
				);
			...
			<>
				<KeyboardAwareScrollView>
					<View
						...
						<Image
							source={{
								uri: "https://cdn.discordapp.com/attachments/650778484523794456/789992516505305098/Sans_titre_-_1.jpg",
							}}
							style={{
								height: 105,
								width: 115,
						...
						<View
							...
							<TouchableOpacity
								...
								<View
									...
									<Text style={{ color: connexion ? "#ffffff" : "#78849E" }}>{I18n.t("connexion")}</Text>
								</View>
							</TouchableOpacity>
							...
							<TouchableOpacity
								...
								<View
									...
									<Text style={{ color: inscription ? "#ffffff" : "#78849E" }}>{I18n.t("inscription")}</Text>
								</View>
							</TouchableOpacity>
						</View>
						...
						/>
						<InputComponent
							name={I18n.t("inscriptionIdentifiant")}
							placeholder={I18n.t("inscriptionDescIdentifiant") + "                                                  "}
							value={state.mail}
							icon={faEnvelope}
							onChange={(v: string) => setState({ ...state, mail: v })}
						/>
						<InputComponent
							password={true}
						...
						<InputComponent
							...
							placeholder={I18n.t("inscriptionCMDP") + "                                                  "}
						...
						<View style={{ marginTop: 5, marginBottom: 5, display: "flex", flexDirection: "row", alignItems: "center" }}>
							...
							<Text
								onPress={() => {
									dispatch(setCacheNav("Connexion"));
						...
						<View
							...
							{errorS && (
								<Text style={{ color: "red", textAlign: "center", marginBottom: 7.5 }}>{I18n.t("inscriprionERR1")}</Text>
							)}
							{errorM && (
								<Text style={{ color: "red", textAlign: "center", marginBottom: 7.5 }}>{I18n.t("inscriprionERR2")}</Text>
							)}
							{errorU && (
								<Text style={{ color: "red", textAlign: "center", marginBottom: 7.5 }}>{I18n.t("inscriprionERR3")}</Text>
							)}
		...
		<>
			<KeyboardAwareScrollView>
				<View
					...
					<InputComponent
						password={true}
						name={I18n.t("connexoinMDP")}
						placeholder={I18n.t("connexoinMDP") + "                                                  "}
						value={state.password}
						icon={faLock}
						onChange={(v: string) => setState({ ...state, password: v })}
					/>
					...
					<View
						...
						<View
							...
							<TouchableOpacity onPress={() => Linking.openURL("http://176.31.230.112:5000/password-reset/new")}>
								<Text
									style={{
										color: "#3497FD",
									}}>
						...
						{error && (
							<Text style={{ color: "red", textAlign: "center", marginBottom: 10 }}>
								Identifiant ou/et mot de passe erronée
							</Text>
						)}
												...
												setTimeout(() => {
													Users.Load(
														(e: any) => {
															...
															dispatch(
																setImage(
																	`http://176.31.230.112:3000/api/avatar/${e.mail}?time=${new Date()}`
																)
															);
