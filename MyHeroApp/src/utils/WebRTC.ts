...
const SIGNALING_SERVER_URL = 'http://146.59.227.90:8843';
const TURN_SERVER_URL = '146.59.227.90:8843';
...
export default class WebRTC {
    ...
    onAddStream = (event: { stream: any; }) => {
      console.log('Add stream');
      this.onRemoteStreamObtained(event.stream);
    };
  
    handleSignalingData = (data: RTCSessionDescriptionType) => {
      switch (data.type) {
        case 'offer':
          this.createPeerConnection();
